...
export function startHostApiServer(ctx: HostApiContext, port = getPort('DPCLAW_HOST_API')): Server {
  ...
  server.on('error', (error: NodeJS.ErrnoException) => {
    if (error.code === 'EACCES' || error.code === 'EADDRINUSE') {
      logger.error(
        ...
        `Set DPCLAW_PORT_DPCLAW_HOST_API env var to override the default port.`,
      );
    } else {
