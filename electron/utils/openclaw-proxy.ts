...
interface SyncProxyOptions {
   ...
   * currently disabled in DPClaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync DPClaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because DPClaw proxy is disabled and preserve mode is enabled');
      return;
    }
