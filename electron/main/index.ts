...
import { ensureDPClawContext, repairDPClawOnlyBootstrapFiles } from '../utils/openclaw-workspace';
...
const WINDOWS_APP_USER_MODEL_ID = 'app.dpclaw.desktop';
const isE2EMode = process.env.DPCLAW_E2E === '1';
const requestedUserDataDir = process.env.DPCLAW_USER_DATA_DIR?.trim();
...
// On Wayland this maps the running window to dpclaw.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('dpclaw.desktop');
}
...
if (!gotElectronLock) {
  console.info('[DPClaw] Another instance already holds the single-instance lock; exiting duplicate process');
  app.exit(0);
}
    ...
    if (!fileLock.acquired) {
      const ownerDescriptor = fileLock.ownerPid
        ...
        : fileLock.ownerFormat === 'unknown'
          ...
          : 'unknown owner';
      console.info(
        `[DPClaw] Another instance already holds process lock (${fileLock.lockPath}, ${ownerDescriptor}); exiting duplicate process`,
      );
      app.exit(0);
    }
  } catch (error) {
    console.warn('[DPClaw] Failed to acquire process instance file lock; continuing with Electron single-instance lock only', error);
  }
}
...
function createWindow(): BrowserWindow {
  ...
  const shouldSkipSetupForE2E = process.env.DPCLAW_E2E_SKIP_SETUP === '1';
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== DPClaw Application Starting ===');
  logger.debug(
  ...
  // Repair any bootstrap files that only contain DPClaw markers (no OpenClaw
  // template content). This fixes a race condition where ensureDPClawContext()
  // previously created the file before the gateway could seed the full template.
  if (!isE2EMode) {
    void repairDPClawOnlyBootstrapFiles().catch((error) => {
  ...
  gatewayManager.on('status', (status: { state: string }) => {
    ...
    if (status.state === 'running' && !isE2EMode) {
      void ensureDPClawContext().catch((error) => {
        logger.warn('Failed to re-merge DPClaw context after gateway reconnect:', error);
      });
    }
  });
  ...
  // Merge DPClaw context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensureDPClawContext will retry until the target files appear.
  if (!isE2EMode) {
    void ensureDPClawContext().catch((error) => {
      logger.warn('Failed to merge DPClaw context into workspace:', error);
    });
  }
...
if (gotTheLock) {
  ...
  app.on('second-instance', () => {
    logger.info('Second DPClaw instance detected; redirecting to the existing window');
