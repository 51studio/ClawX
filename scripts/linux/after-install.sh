...
# Post-installation script for DPClaw on Linux
...
# Create symbolic link for DPClaw app binary
if [ -x /opt/DPClaw/dpclaw ]; then
    ln -sf /opt/DPClaw/dpclaw /usr/local/bin/dpclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/DPClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/DPClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/DPClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/DPClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/dpclaw'
...
echo "DPClaw has been installed successfully."
